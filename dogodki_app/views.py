...
class DogodekView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for skupina in self.object.skupine.all():
            prijavljeni = skupina.prijavljeni.all()
            prijavljeni_oddelek = skupina.prijavljeni.filter(uporabnik__oddelek=self.request.user.oddelek).all()
            ...
            obj_skupina = {
                ...
                "število_prijavljenih_oddelek": prijavljeni_oddelek.count(),
                "prijavljeni": [],
                "moja": False
            }

            obj_skupina["polna"] = obj_skupina["število_mest"] - obj_skupina["število_prijavljenih"] < 1 \
                                   or skupina.omejitev_na_oddelek - obj_skupina["število_prijavljenih_oddelek"] < 1
